...
void uart_init(struct uart *uart, unsigned long baud) {
    ...
    uart->CR1.reg = 0; // Clear CR1 to disable USART and reset settings
...
int uart_read_ready(struct uart *uart) {
    return uart->SR.field.RXNE;  // If RXNE bit is set, data is ready
}

uint8_t uart_read_byte(struct uart *uart) {
    return (uint8_t) (uart->DR.reg & 255);
}

void uart_write_char(struct uart *uart, char ch) {
    while (!uart->SR.field.TXE);  // Wait until TXE bit is set, indicating data register is empty
    uart->DR.reg = (uint32_t)ch;
}
