...
void systick_init(uint32_t ticks) {
    ...
    SYSTICK->CSR.ENABLE = 0;    // Disable SysTick during setup
    SYSTICK->RVR = ticks - 1;   // Set reload register
    SYSTICK->CVR = 0;           // Clear current value register
    SYSTICK->CSR.CLKSOURCE = 1; // Use processor clock
    SYSTICK->CSR.TICKINT = 1;   // Enable SysTick interrupt
    SYSTICK->CSR.ENABLE = 1;    // Enable SysTick timer
