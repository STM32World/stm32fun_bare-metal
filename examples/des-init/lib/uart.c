...
void uart_init(volatile struct uart *uart, unsigned long baud) {
    ...
    if (uart == USART1) {
        ...
        RCC->APB1ENR_b.UART2EN = 1; // Enable USART2 clock
        af = 7;                     // AF7 for USART2
        rx = PIN('A', 3);           // USART2_RX
        tx = PIN('A', 2);           // USART2_TX
        pclk = APB1_FREQUENCY;
    } else if (uart == USART3) {
        RCC->APB1ENR_b.UART3EN = 1; // Enable USART3 clock
        af = 7;                     // AF7 for USART3
        rx = PIN('B', 11);          // USART3_RX
        tx = PIN('B', 10);          // USART3_TX
        pclk = APB1_FREQUENCY;
    } else if (uart == UART4) {
        RCC->APB1ENR_b.UART4EN = 1; // Enable UART4 clock
        af = 8;                     // AF8 for USART4
        rx = PIN('C', 11);          // USART4_RX
        tx = PIN('C', 10);          // USART4_TX
        pclk = APB1_FREQUENCY;
    } else if (uart == UART5) {
        RCC->APB1ENR_b.UART5EN = 1; // Enable UART5 clock
        af = 8;                     // AF8 for UART5
...
uint8_t uart_read_byte(volatile struct uart *uart) {
    return (uint8_t)(uart->DR & 255);
}

void uart_write_char(volatile struct uart *uart, char ch) {
    while (!uart->SR_b.TXE)
